...
Options:
  ...
  --math-jax-url=<url>  Specify a different URL used for loading the MathJax
                        library.
...
def main(argv=None, force_utf8=True, patch_svg=True):
    ...
    if args['--export']:
        try:
            export(args['<path>'], args['--user-content'], args['--context'],
                   ...
                   not args['--no-inline'], args['--render-math'],
                   args['<address>'], args['--api-url'], args['--math-jax-url'],
                   args['--title'], args['--quiet'])
            return 0
    ...
    try:
        serve(path, host, port, args['--user-content'], args['--context'],
              ...
              args['--render-math'], args['--api-url'], args['--math-jax-url'],
              args['--title'], not args['--norefresh'], args['--browser'],
              args['--quiet'], None)
        return 0
    except ReadmeNotFoundError as ex:
        print('Error:', ex)
